...
/* ST SPI header: MSB = 1 for read, 0 for write; remaining 7 bits = address */
...
int main(void) {
    ...
    while (true) {
        ...
        if ((status & 0x03) == 0x03) {
            ...
            gx_dps = gx * GYRO_2000DPS_SENS;
            gy_dps = gy * GYRO_2000DPS_SENS;
            gz_dps = gz * GYRO_2000DPS_SENS;
            ...
            ax_ms2 = ax * ACC_16G_SENS_G * G_TO_MS2;
            ay_ms2 = ay * ACC_16G_SENS_G * G_TO_MS2;
            az_ms2 = az * ACC_16G_SENS_G * G_TO_MS2;
            //printf("G[dps]: %8.2f %8.2f %8.2f | A[m/s^2]: %8.3f %8.3f %8.3f\n",
             //   gx_dps, gy_dps, gz_dps, ax_ms2, ay_ms2, az_ms2);
            printf("Angle: %8.2f %8.2f %8.2f deg\n", angle_x, angle_y, angle_z);
        }
        

        // Keep USB happy; remove to go faster (prints will still bottleneck)
        sleep_ms(1);
    }
}
