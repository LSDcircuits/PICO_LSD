...
void oled_draw_char_scaled(int x, int y, char c, int scale) {
    if (c < 32 || c > 90) return;  // Supported range in your font table
    const uint8_t *glyph = font5x7[c - 32];
...
void oled_draw_string_scaled(int x, int y, const char *str, int scale) {
    while (*str) {
        ...
        x += (5 + 1) * scale;  // 5px glyph + 1px spacing, scaled
    }
}

void oled_print_value_scaled(float value) {
    char buf[16];
    sprintf(buf, "%03.0f", value);

    int scale = 3;  // Try 1, 2, or 3
    int text_width = strlen(buf) * (5 + 1) * scale;
    int x = (OLED_WIDTH - text_width) / 2;
    int y = (OLED_HEIGHT / 2) - (7 * scale / 2);

    oled_clear();
    oled_draw_string_scaled(x, y, buf, scale);
    oled_update();
}
